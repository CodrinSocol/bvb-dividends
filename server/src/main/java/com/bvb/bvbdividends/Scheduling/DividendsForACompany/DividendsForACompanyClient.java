...
public class DividendsForACompanyClient extends WebServiceGatewaySupport {
	private static final String DIVIDENDS_FOR_A_COMPANY_ENDPOINT = "https://ws.bvb.ro/BVBFinancialsWS/Financials.asmx";
	private static final SoapActionCallback DIVIDENDS_FOR_A_COMPANY_CALLBACK = new SoapActionCallback("http://www.bvb.ro/GetDividends");
	...
	public List<Dividend> getDividendsForACompany(Company company) {
		...
		try {
			GetDividendsResponse getDividendsResponse = (GetDividendsResponse) getWebServiceTemplate()
					.marshalSendAndReceive(DIVIDENDS_FOR_A_COMPANY_ENDPOINT, getDividendsForACompanyRequest, DIVIDENDS_FOR_A_COMPANY_CALLBACK);

			return toDividends(company, getDividendsResponse.getGetDividendsResult().getDividends().getDividendInfo());
		} catch (Exception e) {
	...
	private List<Dividend> toDividends(Company company, List<DividendInfo> dividendInfoList) {
		return dividendInfoList.stream().map(dividendInfo -> Dividend.fromDividendInfo(company, dividendInfo)).toList();
	}
}
